...
import {
    ...
    doc, getDoc, getDocs, deleteDoc, writeBatch,
...
export default function Messages({ navigation }) {
    ...
    useEffect(() => {
        ...
        const fetchEverything = async () => {
            setLoadingLikes(true); setLoadingMatches(true); setLoadingChats(true);
            ...
            const likesPromise = (async () => {
                try {
                    ...
                    const myLikes = new Set(fromMeSnap.docs.map((d) => d.data().to));

                    /* atmetame jau esamus match’us */
                    const [m1, m2] = await Promise.all([
                        getDocs(query(collection(db, 'matches'), where('user1', '==', userId))),
                        getDocs(query(collection(db, 'matches'), where('user2', '==', userId))),
                    ]);
            ...
            const matchesPromise = (async () => {
                try {
                    ...
                    const otherUIDs = docs.map((d) =>
                        d.data().user1 === userId ? d.data().user2 : d.data().user1
                    );
                    ...
                    return docs.map((d) => {
                        const otherUid = d.data().user1 === userId ? d.data().user2 : d.data().user1;
            ...
            const listenChats = () => {
                ...
                unsubscribeChats = onSnapshot(q, async (snap) => {
                    try {
                        ...
                        if (!docs.length) { setConversations([]); setLoadingChats(false); return; }

                        const meta      = docs.map((d) => ({ id: d.id, ...d.data() }));
                        const otherUIDs = meta.map((c) => c.participants.find((p) => p !== userId));
                        ...
                        const convs = await Promise.all(meta.map(async (c) => {
                            const oUid = c.participants.find((p) => p !== userId);
            ...
            /* ---------- Rezultatai ---------- */
            const [likesRes, matchesRes] = await Promise.all([likesPromise, matchesPromise]);
            setLikedByUsers(likesRes);   setLoadingLikes(false);
            setMatches(matchesRes);      setLoadingMatches(false);
        ...
        return () => { unsubscribeChats(); unsubFocus(); };
    }, [navigation, userId]);
    ...
    const confirmDelete = (chatId, otherUid) =>
        Alert.alert('Pašalinti pokalbį', 'Ar tikrai nori pašalinti ir unmatchinti?', [
            { text: 'Atšaukti', style: 'cancel' },
            {
                text: 'Taip', style: 'destructive',
                onPress: () => {
                    void deleteChatAndUnmatch(
                        chatId,
                        otherUid,
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContainer}>
                                    ...
                                    <View>
                                        ...
                                        {likedByUsers[0] && (
                                            <View style={styles.iconsOverlay}>
                                                {hasType(likedByUsers[0], 'kambarioko') && <Tag type="kambarioko" />}
                                                {hasType(likedByUsers[0], 'bendraminciu') && <Tag type="bendraminciu" />}
                                            </View>
                                        )}
                                        <View style={styles.likesOverlay}>
                                            <Text style={styles.likesCount}>+{totalPendingLikes}</Text>
                        ...
                        <View style={styles.matchesBlock}>
                            {loadingMatches ? (
                                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                                    {[...Array(3)].map((_, i) => (
                                        <SkeletonBox key={i} width={110} height={170} style={{ marginRight: 12 }} />
                                    ))}
                                </ScrollView>
                            ) : (
                                <ScrollView
                                            ...
                                            <TouchableOpacity
                                                ...
                                                onPress={() =>
                                                    navigation.navigate('WriteMessages', {
                                                        ...
                                                        receiverName:  m.otherName,
                                                        receiverAvatar:m.otherAvatar,
                                                    })
                                                }>
                                                <Image source={{ uri: m.otherAvatar }} style={styles.matchImage} />
                                                <View style={styles.iconsOverlay}>
                                                    {hasType(m, 'kambarioko') && <Tag type="kambarioko" />}
                                                    {hasType(m, 'bendraminciu') && <Tag type="bendraminciu" />}
                                                </View>
                                                <View style={styles.matchOverlay}>
                                                    <Text style={styles.matchOverlayText} numberOfLines={1}>
                                                        {m.otherName}
                                                    </Text>
                                                </View>
                                            </TouchableOpacity>
                ...
                <View style={[styles.sectionContainer, { marginTop: 16 }]}>
                    ...
                    {loadingChats ? (
                        ...
                        <Text style={styles.noConversations}>Nėra jokių pokalbių.</Text>
                    ) : (
                        conversations.map((c) => (
                            <View key={c.id} style={styles.messageRow}>
                                <TouchableOpacity
                                    ...
                                    onLongPress={() => confirmDelete(c.id, c.otherUid)}
                                    delayLongPress={500}>
                                    <Image source={{ uri: c.otherAvatar }} style={styles.avatar} />

                                    <View style={styles.messageTextContainer}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap' }}>
                                            <Text style={styles.messageName}>
                                                {c.otherName}
                                                {c.hasUnread && <Text style={styles.unreadDot}> •</Text>}
                                            </Text>
                                            {hasType(c, 'kambarioko') && <Tag type="kambarioko" />}
                                            {hasType(c, 'bendraminciu') && <Tag type="bendraminciu" />}
                                        </View>
            ...
            <Footer activeTab="Messages" onTabPress={(tab) => navigation.navigate(tab)} />
